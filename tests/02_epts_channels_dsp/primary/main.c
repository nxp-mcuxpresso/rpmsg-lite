 ...
 *****************************************************************************/
void tc_1_defchnl_transport_send(struct rpmsg_lite_endpoint *src, int32_t dst, int32_t send_value, int32_t expected_value)
{
    void *mutex = NULL;
    int32_t result = 0;
    LOCK_STATIC_CONTEXT mutex_static_ctxt;
    result = env_create_mutex(&mutex, 1, &mutex_static_ctxt);
    TEST_ASSERT_MESSAGE(0 == result, "unexpected value");
    result = rpmsg_lite_send(my_rpmsg, src, dst, (char *)&send_value, sizeof(send_value), RL_BLOCK);
...
void tc_1_defchnl_transport(void)
{
    ...
    for (int32_t j = 0; j < TC_TRANSFER_COUNT; j++)
    {
        ...
        {
            // remote has custom epts with the different *address*
            // send data between custom epts with different *address*
            tc_1_defchnl_transport_send(epts[i],                // src addr
                                        TC_REMOTE_EPT_ADDR + i, // dst addr
                                        i,                      // value to send
                                        i + 10                  // expected value to receive
                                        );
        }
    }
...
void run_tests(void *unused)
{
    ...
    __coveragescanner_testname("02_epts_channels");
    __coveragescanner_install("02_epts_channels.csexe");
