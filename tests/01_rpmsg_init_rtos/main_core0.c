...
void tc_1_rpmsg_init()
{
    ...
    my_rpmsg = rpmsg_lite_master_init(rpmsg_lite_base, SH_MEM_TOTAL_SIZE / 4, RPMSG_LITE_LINK_ID, RL_NO_FLAGS);
    ...
    my_rpmsg = rpmsg_lite_master_init(RL_NULL, SH_MEM_TOTAL_SIZE, RPMSG_LITE_LINK_ID, RL_NO_FLAGS);
    ...
    TEST_ASSERT_MESSAGE(RL_NULL == my_rpmsg, "init function with bad link_id param failed");
    /* Wrong shmem_addr param */
    my_rpmsg = rpmsg_lite_master_init(RL_NULL, RPMSG_LITE_SHMEM_SIZE, RPMSG_LITE_LINK_ID, RL_NO_FLAGS);
    TEST_ASSERT_MESSAGE(RL_NULL == my_rpmsg, "init function with bad shmem_addr param failed");
#endif /* SH_MEM_NOT_TAKEN_FROM_LINKER */
...
void tc_2_env_testing()
{
    ...
    TEST_ASSERT_MESSAGE(-1 == env_create_sync_lock(&mutex, 100),
                        "env_create_sync_lock function with bad count param failed");
    ...
    // virtqueue_free testing, reach cases when (vq == VQ_NULL)
