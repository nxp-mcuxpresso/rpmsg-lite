...
void run_tests(void *unused)
{
    ...
    TEST_ASSERT_MESSAGE(0 == env_create_mutex(&aux_mutex, 1), "env_create_mutex function failed");
    TEST_ASSERT_MESSAGE(0 == env_create_queue(&aux_q, 1, sizeof(uint32_t)), "env_create_queue function failed");
