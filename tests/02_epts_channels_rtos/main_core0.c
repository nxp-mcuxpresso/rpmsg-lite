...
#else
    my_rpmsg = rpmsg_lite_master_init((void *)RPMSG_LITE_SHMEM_BASE, RPMSG_LITE_SHMEM_SIZE, RPMSG_LITE_LINK_ID, RL_NO_FLAGS);
#endif /* SH_MEM_NOT_TAKEN_FROM_LINKER */
...
int32_t ts_create_epts(rpmsg_queue_handle queues[], struct rpmsg_lite_endpoint *volatile epts[], int32_t count, int32_t init_addr)
{
    ...
    TEST_ASSERT_MESSAGE(RL_NULL == rpmsg_lite_create_ept(RL_NULL, init_addr, rpmsg_queue_rx_cb, NULL), "'rpmsg_lite_create_ept' with bad rpmsg_lite_dev param failed");
    return 0;
}
...
void tc_1_defchnl_1(void)
{
    ...
    result = ts_create_epts(&default_queue, &default_ept, 1, TC_LOCAL_EPT_ADDR);
...
void tc_1_defchnl_3(void)
{
    ...
    result = ts_create_epts(queues, epts, TC_EPT_COUNT, TC_LOCAL_EPT_ADDR);
    TEST_ASSERT_MESSAGE(0 == result, "'ts_create_epts' failed");
    if (result)
        goto end1;
