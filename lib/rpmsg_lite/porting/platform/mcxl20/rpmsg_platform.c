...
int32_t MU_A_INT_IRQHandler(void)
{
    mu_isr(MUA);
    return 0;
}
#elif defined(FSL_FEATURE_MU_SIDE_B)
int32_t MU_B_INT_IRQHandler(void)
{
    mu_isr(MUB);
    return 0;
}
...
int32_t platform_init_interrupt(uint32_t vector_id, void *isr_data)
{
    if (platform_lock != ((void *)0))
    {
        ...
        {
#if defined(FSL_FEATURE_MU_SIDE_A)
            MU_EnableInterrupts(MUA, MU_GI_INTR(1UL << vector_id));
#elif defined(FSL_FEATURE_MU_SIDE_B)
            MU_EnableInterrupts(MUB, MU_GI_INTR(1UL << vector_id));
#endif
        }
        isr_counter++;
...
int32_t platform_deinit_interrupt(uint32_t vector_id)
{
    if (platform_lock != ((void *)0))
    {
        ...
        {
#if defined(FSL_FEATURE_MU_SIDE_A)
            MU_DisableInterrupts(MUA, MU_GI_INTR(1UL << vector_id));
#elif defined(FSL_FEATURE_MU_SIDE_B)
            MU_DisableInterrupts(MUB, MU_GI_INTR(1UL << vector_id));
#endif
        }
...
#if defined(FSL_FEATURE_MU_SIDE_A)
    (void)MU_TriggerInterrupts(MUA, MU_GI_INTR(1UL << (RL_GET_Q_ID(vector_id))));
#elif defined(FSL_FEATURE_MU_SIDE_B)
    (void)MU_TriggerInterrupts(MUB, MU_GI_INTR(1UL << (RL_GET_Q_ID(vector_id))));
#endif
#endif
    env_unlock_mutex(platform_lock);
}
 ...
 * platform_vatopa
 *
 * Translate cm0plus addresses to cm33 addresses
 *
 */
 ...
 * platform_patova
 *
 * Translate cm33 addresses to cm0plus addresses
 *
 */
...
uintptr_t platform_vatopa(void *addr)
{
    return (((uintptr_t)(char *)addr & 0x00FFFFFFu));
}
...
void *platform_patova(uintptr_t addr)
{
    return ((void *)(char *)((addr & 0x00FFFFFFu)));
}
